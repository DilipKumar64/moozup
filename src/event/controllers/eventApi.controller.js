...
const { createDefaultFeatureSettingsForEvent } = require("../../services/featureSetting.service");
...
exports.createEvent = async (req, res) => {
  try {
    ...
    await createDefaultFeatureSettingsForEvent(newEvent.id, Number(creatorId));
...
exports.getEventUsers = async (req, res) => {
  try {
    ...
    if (!users) {
      return res.status(404).json({ error: "Event not found or no users linked" });
    }

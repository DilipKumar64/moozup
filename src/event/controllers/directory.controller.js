...
exports.createDirectoryUser = async (req, res) => {
  ...
  try {
    ...
    const participationType = await findParticipationTypeById(parseInt(participationTypeId));
    ...
    // ✅ Step 1: Create user WITHOUT connecting to event
    const user = await createUser({
      firstName,
      email,
      phoneNumber,
      participationType: { connect: { id: parseInt(participationTypeId) } },
      companyName,
      jobTitle,
      facebookUrl,
      linkedinUrl,
    ...
    // ✅ Step 2: Manually link user and event via UserEvent
    await prisma.userEvent.create({
      data: {
        user: { connect: { id: user.id } },
        event: { connect: { id: parseInt(eventId) } },
        userType: "EVENT" // ya "COMMUNITY" — jo bhi logic hai tera
      }
    });
    ...
    const mailOptions = {
      ...
      to: email,
      subject: `Welcome to Moozup, ${firstName}! Your login details inside 🔥`,
      html: `
        <div style="max-width:600px; margin:auto; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background:#f9f9f9; padding:20px; border-radius:12px; border:1px solid #ddd;">
          <header style="background:#007bff; color:white; padding:15px; border-radius:12px 12px 0 0; text-align:center;">
            <h1>Welcome, ${firstName}!</h1>
          </header>
          <main style="padding:20px; background:white; border-radius:0 0 12px 12px;">
            <p style="font-size:16px; color:#333;">
              Here's your login password and contact info to get you started:
            </p>
            <table style="width:100%; margin-top:20px; font-size:16px; color:#555;">
              <tr>
                <td style="font-weight:bold; padding:8px 0;">Password:</td>
                <td style="font-family: monospace; background:#f0f0f0; padding:6px 10px; border-radius:5px;">${generatedPassword}</td>
              </tr>
              <tr>
            ...
            <p style="margin-top:30px; font-size:16px; color:#333;">
              Click the button below to log in and start your journey:
            </p>
            <div style="text-align:center; margin-top:20px;">
              <a href="https://yourdomain.com/login" style="background:#007bff; color:white; padding:12px 30px; text-decoration:none; border-radius:8px; font-weight:bold; font-size:16px;">
                Login Now
              </a>
            </div>
    ...
    res.status(201).json({
      message: "User created successfully with event and welcome email sent",
      user: {
        ...user,
        password: undefined
      }

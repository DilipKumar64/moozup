...
const getEventUsers = async (eventId) => {
  const eventWithUserEvents = await prisma.event.findUnique({
    where: { id: Number(eventId) },
    include: {
      UserEvent: {       // Your schema uses "UserEvent"
        include: { user: true }
      }
    }
  });

  if (!eventWithUserEvents) return null;

  // Pull users out of UserEvent join table
  const users = eventWithUserEvents.UserEvent.map(ue => ue.user);
  return users;
};
